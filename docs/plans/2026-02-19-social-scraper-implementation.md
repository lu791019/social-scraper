...
**Goal:** 從 Google Sheet 讀取 IG/Threads URL，用 Patchright 爬取內容（文字 + 圖片 OCR + 影片逐字稿），Claude 產摘要，寫回 Sheet。

**Architecture:** Patchright 無頭瀏覽器開貼文頁面，從嵌入 JSON 提取結構化資料（caption、圖片 URL、影片 URL）。圖片用 `claude --print` 做 OCR（走 Max 額度），影片用 ffmpeg + mlx-whisper 本地轉逐字稿。全部文字彙整後由 `claude --print` 產出摘要，透過 gspread 寫回 Google Sheet。**零 API 成本**。

**Tech Stack:** Python 3.12+, Patchright (async), gspread, `claude --print` (Claude Max 額度), mlx-whisper (本地 STT), ffmpeg, httpx, uv
...
- Create: `tests/fixtures/ig_post.json` (spike 取得)
...
- Create: `tests/fixtures/threads_post.json` (spike 取得)
...
git commit -m "feat: 圖片 OCR 模組，用 claude --print 擷取圖片文字（走 Max 額度）"
```

---

### Task 6: 影片逐字稿模組（ffmpeg + mlx-whisper）
...
git commit -m "feat: 影片逐字稿模組，ffmpeg 抽音軌 + mlx-whisper 本地 STT"
```
...
git commit -m "feat: LLM 摘要模組，用 claude --print 產出摘要（走 Max 額度）"
```
...
| 5 | 圖片 OCR（claude --print） | 15 min |
| 6 | 影片逐字稿（mlx-whisper 本地） | 15 min |
| 7 | Google Sheet 讀寫 | 15 min |
| 8 | LLM 摘要（claude --print） | 10 min |
| 9 | 主控流程 | 15 min |
| 10 | 端對端整合測試 | 20 min |

**前置條件 checklist：**
- [x] `ffmpeg` 已安裝（v8.0.1）
- [x] Google Cloud Service Account 已建立，`credentials.json` 已就位
- [x] Google Sheet 已建立並共享給 Service Account
- [x] `.env` 已建立（GOOGLE_SHEET_URL + credentials path）
- [ ] Patchright 瀏覽器已安裝（`uv run patchright install chromium`）
- [x] `claude` CLI 可用且已登入 Max 訂閱
- [ ] mlx-whisper 模型首次下載（Task 6 執行時自動下載，約 1.5GB）
